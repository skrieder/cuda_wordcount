...
#define LIMIT 65536//default: 65536
...
//	table[key] = table[key] + 1; //This don't consider using lock.
...
void copy_table_to_host( const unsigned int* dev_table, unsigned int* hostTable) {
	hostTable = (unsigned int*)calloc(LIMIT, sizeof(unsigned int));
	HANDLE_ERROR( cudaMemcpy( hostTable, dev_table, LIMIT * sizeof( unsigned int ), cudaMemcpyDeviceToHost ) );
...
void verify_table( const unsigned int* dev_table ) {
 // Table   table;
  ...
  // move table to host
  //copy_table_to_host( dev_table, host_table );
...
int main( void ) {
  ...
  //printf("After memset\n");
  printf("On the host:\n");
  printf("The buffer[0] = %u\n", buffer[0]);
  printf("The buffer[1] = %u\n", buffer[1]);
  printf("The buffer[63] = %u\n", buffer[63]);
 ...
 // Table table;
	//unsigned int dev_table[LIMIT];
//  initialize_table( table, HASH_ENTRIES, ELEMENTS );
	//initialize_table( dev_table, LIMIT, LIMIT );
...
//  HANDLE_ERROR( cudaEventRecord( start, 0 ) );
  ...
  add_to_table<<<512,128>>>( dev_keys, dev_values, dev_table, dev_lock );
